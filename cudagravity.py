...
def get_accelerations(positions, softening_factor):
    ...
    cu_get_accelerations[
        get_kernal_parameters(n_particles, 2)
    ](positions, accelerations, softening_factor)

    return accelerations


@cuda.jit('void(float64[:, :], float64[:, :], float64)')
def cu_get_accelerations(positions, accelerations, softening_factor):
    n = len(positions)
    ...
    if i != j and i < n and j < n:
        ...
        drPowerN32 = nb.float32(1.0) / (math.sqrt(drSquared) * drSquared)
